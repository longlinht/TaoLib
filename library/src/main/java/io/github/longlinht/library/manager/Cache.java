...
public class Cache<T> {
	...
	public void save(String key, T value) {
		...
		sp.edit().remove(key).putString(key, GsonUtils.toJson(value)).commit();
	}
	...
	public T get(String key) {
		...
		return GsonUtils.fromJson(sp.getString(StringUtil.getTrimedString(key), null), clazz);
	}
	...
	public List<T> getAllValueList() {
		...
		if (map != null) {
			...
			for (String value : map.values()) {
				...
				data = GsonUtils.fromJson(value, clazz);
				if (data != null) {
					list.add(data);
				}
			}
